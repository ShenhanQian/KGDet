...
class KeypointTransform(object):
    ...
    1. resize keypoints to expected size and stack to a single array
    2. flip the keypoints (if needed)
    """

    def __call__(self, keypoints, img_shape, pad_shape, gt_labels,
                 scale_factor, flip_pairs, flip=False):

        h1, w1, _ = img_shape
        h2, w2, _ = pad_shape
        keypoints = [
            np.c_[keypoint[:, 0:2] * scale_factor, keypoint[:, 2]]
            for keypoint in keypoints
        ]

        if flip:
            keypoints = [fliplr_joints(keypoint, w1, flip_pairs[gt_label-1])
                         for gt_label, keypoint in zip(gt_labels, keypoints)]
