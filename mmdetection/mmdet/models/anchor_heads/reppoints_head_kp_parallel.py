...
class RepPointsHeadKpParallel(nn.Module):
    """RepPoint head.
    ...
    def _init_layers(self):
        ...
        self.reppts_init_conv = nn.Conv2d(self.feat_channels,
                                          self.point_feat_channels, 3,
                                          1, 1)
        self.reppts_init_out = nn.Conv2d(self.point_feat_channels,
        ...
        self.reppts_refine_dfmconv = DeformConv(self.feat_channels,
                                                ...
                                                self.dcn_kernel, 1,
                                                self.dcn_pad)
        self.reppts_refine_out = nn.Conv2d(self.point_feat_channels,
                                           reppts_out_dim, 1, 1, 0)
    ...
    def forward_single(self, x):
        ...
        reppts_out_init = self.reppts_init_out(
            self.relu(self.reppts_init_conv(pts_feat)))
        ...
        reppts_out_refine = self.reppts_refine_out(
            self.relu(self.reppts_refine_dfmconv(pts_feat, dcn_offset)))
        if self.use_grid_points:
            pass
    ...
    def loss(self,
        ...
        (losses_cls, losses_bbox_init, losses_bbox_refine,
         losses_kpt_init, losses_kpt_refine) = multi_apply(
            ...
            keypts_coordinate_preds_refine,
            reppts_coordinate_preds_init,
            reppts_coordinate_preds_refine,
            labels_list,
            label_weights_list,
            bbox_gt_list_init,
            ...
            keypoint_gt_list_refine,
            keypoint_weights_list_refine,
            self.point_strides,
