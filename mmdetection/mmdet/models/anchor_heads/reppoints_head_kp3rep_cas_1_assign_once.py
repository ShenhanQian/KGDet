...
class Kp3RepBlock(nn.Module):
    ...
    def __init__(self,
        ...
        if deform_conv:
            ...
            self.cls_dfmconv_3 = DeformConv(in_channels, feat_channels,
                                            self.dcn_kernel_3, 1,
                                            self.dcn_pad_3)
            self.cls_dfmconv_5 = DeformConv(in_channels, feat_channels,
                                            self.dcn_kernel_5, 1,
                                            self.dcn_pad_5)
            self.cls_dfmconv_7 = DeformConv(in_channels, feat_channels,
                                            self.dcn_kernel_7, 1,
                                            self.dcn_pad_7)
            self.cls_out = nn.Conv2d(feat_channels*3, cls_out_channels,
                                     1, 1, 0)
            # self.cls_out = nn.Conv2d(feat_channels, cls_out_channels,
            #                          1, 1, 0)
            self.keypts_dfmconv_3 = DeformConv(in_channels, feat_channels,
                                               self.dcn_kernel_3, 1,
                                               self.dcn_pad_3)
            self.keypts_dfmconv_5 = DeformConv(in_channels, feat_channels,
                                               self.dcn_kernel_5, 1,
                                               self.dcn_pad_5)
            self.keypts_dfmconv_7 = DeformConv(in_channels, feat_channels,
                                               self.dcn_kernel_7, 1,
                                               self.dcn_pad_7)
            self.keypts_out = nn.Conv2d(feat_channels*3, keypts_out_dim,
                                        1, 1, 0)
        ...
        if self.deform_conv:
            normal_init(self.cls_dfmconv_3, std=0.01)
            normal_init(self.cls_dfmconv_5, std=0.01)
            normal_init(self.cls_dfmconv_7, std=0.01)
            normal_init(self.keypts_dfmconv_3, std=0.01)
            normal_init(self.keypts_dfmconv_5, std=0.01)
            normal_init(self.keypts_dfmconv_7, std=0.01)
        else:
    ...
    def forward(self, cls_feat, pts_feat, reppts_offset=None):
        if self.deform_conv:
            dcn_base_offset_3 = self.dcn_base_offset_3.type_as(pts_feat)
            dcn_base_offset_5 = self.dcn_base_offset_5.type_as(pts_feat)
            dcn_base_offset_7 = self.dcn_base_offset_7.type_as(pts_feat)

            reppts_offset_3 = reppts_offset[:, :2*9, :, :]
            reppts_offset_5 = reppts_offset[:, 2*9:2*(9+25), :, :]
            reppts_offset_7 = reppts_offset[:, 2*(9+25):2*(9+25+49), :, :]

            reppts_offset_grad_mul_3 = self.gradient_mul * reppts_offset_3 \
                + (1 - self.gradient_mul) * reppts_offset_3.detach()
            dcn_offset_3 = reppts_offset_grad_mul_3 - dcn_base_offset_3
            reppts_offset_grad_mul_5 = self.gradient_mul * reppts_offset_5 \
                + (1 - self.gradient_mul) * reppts_offset_5.detach()
            dcn_offset_5 = reppts_offset_grad_mul_5 - dcn_base_offset_5
            reppts_offset_grad_mul_7 = self.gradient_mul * reppts_offset_7 \
                + (1 - self.gradient_mul) * reppts_offset_7.detach()
            dcn_offset_7 = reppts_offset_grad_mul_7 - dcn_base_offset_7

            cls_dfmconv_feat_3 = self.relu(
                self.cls_dfmconv_3(cls_feat, dcn_offset_3))
            cls_dfmconv_feat_5 = self.relu(
                self.cls_dfmconv_5(cls_feat, dcn_offset_5))
            cls_dfmconv_feat_7 = self.relu(
                self.cls_dfmconv_7(cls_feat, dcn_offset_7))
            cls_dfmconv_feat = torch.cat(
                [cls_dfmconv_feat_3, cls_dfmconv_feat_5, cls_dfmconv_feat_7],
                dim=1)
            # cls_dfmconv_feat = cls_dfmconv_feat_3 \
            #     + cls_dfmconv_feat_5 + cls_dfmconv_feat_7
            cls_out = self.cls_out(cls_dfmconv_feat)

            keypts_dfmconv_feat_3 = self.relu(
                self.keypts_dfmconv_3(pts_feat, dcn_offset_3))
            keypts_dfmconv_feat_5 = self.relu(
                self.keypts_dfmconv_5(pts_feat, dcn_offset_5))
            keypts_dfmconv_feat_7 = self.relu(
                self.keypts_dfmconv_7(pts_feat, dcn_offset_7))
            keypts_dfmconv_feat = torch.cat(
                [keypts_dfmconv_feat_3, keypts_dfmconv_feat_5,
                 keypts_dfmconv_feat_7], dim=1)
            # keypts_dfmconv_feat = keypts_dfmconv_feat_3 \
            #     + keypts_dfmconv_feat_5 + keypts_dfmconv_feat_7
...
class RepPointsHeadKp3RepCas1AssignOnce(nn.Module):
    """RepPoint head.
        ...
        for i in range(self.stacked_convs):
            ...
            self.reg_convs.append(
                ConvModule(
                    ...
                    norm_cfg=self.norm_cfg))
        # stage 1
        self.kp_rep_block_1 = Kp3RepBlock(False, self.cls_out_channels,
                                          self.feat_channels,
                                          ...
                                          self.gradient_mul)
        # stage 2
        self.kp_rep_block_2 = Kp3RepBlock(True, self.cls_out_channels,
                                          self.feat_channels,
                                          self.point_feat_channels,
                                          self.num_reppts,
                                          self.num_keypts,
                                          self.gradient_mul)
        # stage 3
        self.kp_rep_block_3 = Kp3RepBlock(True, self.cls_out_channels,
                                          self.feat_channels,
                                          self.point_feat_channels,
                                          self.num_reppts,
                                          self.num_keypts,
